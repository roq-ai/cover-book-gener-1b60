...
function ReviewViewPage() {
  ...
  const queryParams = useMemo<{ include: { user: boolean; cover_book: boolean } }>(
    () =>
      convertQueryToPrismaUtil(
        {
          relations: ['user', 'cover_book'],
          id,
        },
        'review',
      ),
          ...
          <>
            <FormWrapper wrapperProps={{ border: 'none', gap: 3, p: 0 }}>
              ...
              >
                ...
                {hasAccess('cover_book', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                  <FormListItem
                    label="Cover Book"
                    text={
                      <Link as={NextLink} href={`/cover-books/view/${data?.cover_book?.id}`}>
                        {data?.cover_book?.title + ''}
                      </Link>
                    }
                  />
                )}
