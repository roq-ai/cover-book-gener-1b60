...
import { useGenreCount, useLanguageCount, useReviewCount } from 'lib/roq';

import { CoverBookListPage as ShowCaseList } from 'pages/cover-books';

function DashboardPage() {
  const { session } = useSession();

  const genreStat = useGenreCount();

  const languageStat = useLanguageCount();
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      title: 'Genres',
      result: genreStat?.data,
      isLoading: genreStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Languages',
      result: languageStat?.data,
      isLoading: languageStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Reviews',
          ...
          <Stack direction="column" spacing={8} flex={{ base: 1, md: 7 }}>
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                ...
                <NextLink href={`cover-books`} passHref legacyBehavior>
                  <Button
