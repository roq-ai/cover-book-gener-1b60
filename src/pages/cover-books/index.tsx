...
import { useRoqClient, useCoverBookFindManyWithCount } from 'lib/roq';
import { convertQueryToPrismaUtil } from 'lib/utils';

type ColumnType = ColumnDef<CoverBookInterface, unknown>;

interface CoverBookListPageProps {
...
export function CoverBookListPage(props: CoverBookListPageProps) {
  const {
    filters = {},
    titleProps = {},
  ...
  const queryParams = useMemo(
    () =>
      convertQueryToPrismaUtil(
        {
          relations: ['user', 'company', 'review.count'],
          searchTerm: params.searchTerm,
          searchTermKeys: [
            'title.contains',
            'author.contains',
            'cover_book_genre.contains',
            'cover_book_language.contains',
          ],
          ...params.filters,
        },
        'cover_book',
  ...
  const { data, error, isLoading, mutate } = useCoverBookFindManyWithCount({
  ...
  const handleDelete = async (id: string) => {
    setDeleteError(null);
    try {
      await roqClient.cover_book.delete({ where: { id } });
      await mutate();
    } catch (error) {
  ...
  const handleView = (row: CoverBookInterface) => {
    if (hasAccess('cover_book', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/cover-books/view/${row.id}`);
    }
  };
  ...
  const columns: ColumnType[] = [
    ...
    { id: 'publish_year', header: 'publish_year', accessorKey: 'publish_year' },
    { id: 'cover_book_genre', header: 'Cover Book Genre', accessorKey: 'cover_book_genre' },
    { id: 'cover_book_language', header: 'Cover Book Language', accessorKey: 'cover_book_language' },
      ...
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Flex justifyContent="flex-end">
              <NextLink href={`/cover-books/view/${record.id}`} passHref legacyBehavior>
                <Button
              ...
              {hasAccess('cover_book', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                <NextLink href={`/cover-books/edit/${record.id}`} passHref legacyBehavior>
                  <Button
                    onClick={(e) => e.stopPropagation()}
                    mr={2}
                    padding="0rem 0.5rem"
              ...
              {hasAccess('cover_book', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
                <IconButton
                  onClick={(e) => {
                    e.stopPropagation();
    ...
    <Flex direction="column" gap={{ md: 6, base: 7 }} shadow="none">
      <Flex justifyContent={{ md: 'space-between' }} direction={{ base: 'column', md: 'row' }} gap={{ base: '28px' }}>
        <Flex alignItems="center" gap={1}>
          ...
          <AccessInfo entity="cover_book" />
        </Flex>

        {hasAccess('cover_book', AccessOperationEnum.CREATE, AccessServiceEnum.PROJECT) && (
          <NextLink href={`/cover-books/create`} passHref legacyBehavior>
            <Button
              onClick={(e) => e.stopPropagation()}
              height={'2rem'}
